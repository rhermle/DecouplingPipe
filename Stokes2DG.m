function [ P U V X Y] = Stokes2DG(g, M, p0, mu, toGraph )
...
for i=2:M-1
    for j=2:M-1
        ...
        b(index) = d * (p(sub2ind([M,M], i,j+1)) - p(sub2ind([M,M], i,j-1)))/(2 * mu) + (d^2 * g) / mu;      
    end
end
